  ...
  <div>
      ...
      <div class="container">
        ...
        >
          ...
          <el-table-column label="教工号">
            <template slot-scope="scope">{{ scope.row.tno }}</template>
          </el-table-column>
          <el-table-column label="姓名">
            <template slot-scope="scope">{{ scope.row.tname }}</template>
          </el-table-column>
          <el-table-column label="所在院系" width="250px">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="danger" @click="DelTeacher()">我要删除</el-button>
...
import { GetTeacher, DelTeacherOne, DelTeacherList } from "@/axios/api/Teacher"
...
export default {
  ...
  methods: {
    async Refresh() {
      await GetTeacher(this.queryParam).then((res) => {
    ...
    async DelTeacher() {
      if (this.deleteList) {
        ...
        await DelTeacherList(params).then(
          (res) => {
            if (res.data.code == 200) {
        ...
        //删除id
        await DelTeacherOne(this.DelOneId).then((res) => {
          if (res.data.code == 200) {
            this.$message.success({
              message: "<strong>" + res.data.message + "~</strong>",
              dangerouslyUseHTMLString: true,
